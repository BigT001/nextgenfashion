...
import { getNotificationsAction, markNotificationAsReadAction } from "@/modules/dashboard/actions/notifications.actions";
...
export function Navbar() {
  ...
  const handleNotificationClick = async (n: any) => {
    ...
    if (n.id.startsWith("email-")) {
      await markNotificationAsReadAction(n.id);
    }
  };

  const visibleNotifications = notifications.filter((n) => !readNotifIds.includes(n.id));
  ...
  return (
    <header className="h-20 w-full flex items-center gap-6 px-6 bg-white/50 dark:bg-zinc-950/50 backdrop-blur-2xl border-b-2 border-brand-navy/5 shadow-sm sticky top-0 z-40">
      ...
      <div className="flex items-center gap-4">
        {/* Bell / Notifications - Visible only to admin users, not staff */}
        {isAdmin && (
          <>
                ...
                <div className="max-h-[420px] overflow-y-auto">
                  {notifLoading ? (
                    ...
                    <div className="py-10 text-center flex flex-col items-center gap-3">
                      ...
                      <p className="text-xs font-bold text-muted-foreground">No unread notifications</p>
                    </div>
                  ) : (
                    <div className="p-2 space-y-1">
