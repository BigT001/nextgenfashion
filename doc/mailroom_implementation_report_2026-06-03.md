...
- Email Queries: Created a robust data access layer for fetching inbox messages, sent messages, and managing campaign CRUD operations.
- Email Services: Implemented the core business logic using the Resend SDK for dispatching direct emails and batch campaigns.
- Server Actions: Created secure server actions to connect the frontend UI with the backend services.
...
- Functionality:
  ...
  - Dynamically fetches the full email payload (HTML, Text, Sender details) via the Resend receiving API.
